...
| **UI 层次感不足** | 各区域独立背景，缺乏整体统一性 | 移除区域背景，统一使用透明浮动设计 |
| **窗口控制不当** | 固定尺寸应用提供最大化按钮，用户体验不佳 | 移除最大化按钮，保留最小化和关闭功能 |

### UI 面板规划

| Tab | 当前状态 | 功能 |
|-----|---------|------|
| 监控 | ✅ 已完成 | 实时日志流、启动/停止按钮、模型/模式芯片、自定义窗口控件、模型快速切换 |
| 配置 | ✅ 已完成 | 多模型管理、OCR 配置、防风控配置、屏幕坐标一键校准 |
| 人设 | 🚧 占位 | 联系人管理、专属 Prompt 编辑、独立上下文配置 |

### 界面设计系统（M13）

#### 设计理念
- **一体化无边框设计**：移除各区域独立背景，整个应用显示为一体
- **浮动元素美学**：所有交互元素悬浮在统一背景上，营造层次感
- **高级浅色渐变**：现代化配色方案，提供舒适的视觉体验
- **玻璃拟态升级**：增强毛玻璃效果和透明度，提升精致感

#### 配色系统
```css
/* 主色调 - 现代化 Indigo 系列 */
--accent-blue: #6366f1;      /* 主要操作色 */
--accent-purple: #8b5cf6;    /* 次要强调色 */
--accent-green: #10b981;     /* 成功状态 */
--accent-pink: #ec4899;      /* 危险操作 */
--accent-orange: #f59e0b;    /* 警告提示 */
--accent-teal: #14b8a6;      /* 信息提示 */

/* 背景系统 - 高级浅色渐变 */
--bg-unified: linear-gradient(135deg,
  #fafbff 0%, #f8f9fc 25%, #f0f4f8 50%, #f5f7fa 75%, #fafbff 100%);
...
--bg-texture:
  ...
  radial-gradient(ellipse at 40% 60%, rgba(236, 72, 153, 0.02) 0%, transparent 50%),
  linear-gradient(180deg, rgba(255,255,255,0.4) 0%, rgba(255,255,255,0.1) 100%);

/* 微妙噪点纹理 */
noise-filter: fractalNoise baseFrequency="0.9" numOctaves="4";
```

#### 交互效果
- **日志卡片**：毛玻璃背景 + hover 上浮 2px
- **按钮**：深度阴影 + hover 上浮 2px + 缩放微动
- **芯片**：毛玻璃效果 + hover 上浮 1px
- **模型项**：浮动卡片 + 编辑/删除操作区

#### 窗口控制优化
- 移除不适合固定尺寸应用的最大化按钮
- 保留 macOS 风格的最小化和关闭按钮
- 圆形按钮设计，hover 时显示操作图标
...
### 最新修复 (2026-03-23)
**Bug修复**：
1. ✅ **默认人设无法编辑**：修复了 `saveContact()` 函数中 `name` 字段错误使用联系人名称的问题
2. ✅ **模板加载失败**：添加了页面初始化时的模板加载逻辑，确保模板正常显示
3. ✅ **人设字段混淆**：区分了模板模式（name=联系人名称）和自定义模式（name=人设名称）

**功能增强**：
1. ✅ **复制配置功能**：新增 `copyContact()` 函数，支持快速将同一人设应用到多个联系人
2. ✅ **模板系统优化**：`get_persona_templates_list()` 现在返回内置模板 + 用户自定义模板的合并列表
